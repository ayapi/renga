...
into the pane's PTY (trailing space, **no Enter**). Review, optionally add flags (`/foo`, a file path, etc.), press Enter yourself to run. Works identically across bash / zsh / fish / pwsh / cmd.exe because the keystroke is a pre-PTY write, not a shell hook. `--permission-mode bypassPermissions` skips Claude's per-launch permission prompt so peer-launched Claude panes come up ready to work; drop the trailing flag before pressing Enter if you want the prompt back for that pane.
